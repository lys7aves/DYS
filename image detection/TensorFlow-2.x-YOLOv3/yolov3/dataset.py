...
class Dataset(object):
    ...
    def load_annotations(self, dataset_type):
        ...
        for annotation in annotations:
            ...
            #chiyun write
            idx=annotation.find('.jpg')
            im_path=annotation[0:idx+4]
            annotation=annotation[idx+4:]
            ...
            if not os.path.exists(image_path):
                raise KeyError("%s does not exist ... " %image_path)
            if TRAIN_LOAD_IMAGES_TO_RAM:
                # image = cv2.imread(image_path)                     # image_path에 한글이 있을 시 오류 아래 두 줄로 바꿈
                img_array = np.fromfile(image_path, np.uint8)  # image_path에 한글이 있을 시 사용
                image = cv2.imdecode(img_array, cv2.IMREAD_COLOR)  # image_path에 한글이 있을 시 사용
            else:
                image = ''
